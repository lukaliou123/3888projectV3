...
if (notdefault !== ' '){
    notdefault = notdefault.split("-");
    date.setFullYear(notdefault[0]);
    date.setMonth(notdefault[1]-1);
    date.setDate(notdefault[2]);
}
document.querySelector(".event h2").innerHTML = days[date.getDay()]+ ", " + date.getDate() + " " + months[date.getMonth()] + " " + date.getFullYear();
gen_calendar();

if (notdefault !== ' '){
    dateselected(notdefault[2]);
}else{
    getEvent();
}
    ...
    if (type === "coach"){
        ...
        times += `<div class="add_time timecard">
                    <form method = 'post' style="margin-left:5%">
                      ...
                      <button type="submit" class = "notpickbutton" name="addbutton" value="${formatDate}" style="margin-left:29%; width:80px;">Add time</button>
                      <li style="display: inline-block"> type: </li>
                      <select name="event" style="width:33%; ">
...
function gen_calendar(){
    ...
    for (let i = 1; i < monthdays+1; i++){
        if (i === today.getDate() && date.getMonth() === today.getMonth() && date.getFullYear() === today.getFullYear()){
            day += `<p id="${i}" onclick="dateselected(this.id)" class="day today selected">${i}</p>`;
        } else {
            day += `<p id="${i}" onclick="dateselected(this.id)" class="day">${i}</p>`;
        }
